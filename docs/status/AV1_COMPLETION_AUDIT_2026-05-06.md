...
| Vulkan AV1 encode | `scripts/check_vulkan_av1_encode_bindings.rs --fail-on-missing`; `cargo info ash`; `output/vulkan-av1-encode-bindings/vulkan-av1-encode-bindings-1778081150.md` | crates.io still reports `ash 0.38.0+1.3.281`; the local source exposes AV1 decode bindings but no `VK_KHR_video_encode_av1` / `VideoEncodeAV1` / `ENCODE_AV1` bindings, and the fail-on-missing gate exits nonzero as expected | Blocked |
...
| VideoToolbox AV1 | `crates/video-hw-backend-vt/src/vt_backend.rs`; `crates/video-hw-core/src/lib.rs`; `crates/video-hw-fmp4/src/fmp4_reader/decode.rs`; `crates/video-hw/examples/decode_to_yuv.rs`; `scripts/benchmark_ffmpeg_vt_precise.rs`; `scripts/run_vt_precise_suite.rs`; `cargo check -p video-hw-backend-vt --target x86_64-apple-darwin --features backend-vt --tests`; `cargo test -p video-hw-fmp4 --features "backend-nvidia backend-intel backend-vulkan" av1_track_options_preserve_fmp4_av1c_record`; `cargo +nightly -Zscript scripts/benchmark_ffmpeg_vt_precise.rs --help`; `cargo +nightly -Zscript scripts/run_vt_precise_suite.rs --help` | fMP4 AV1 decode bootstrap is scaffolded for VT by passing `av1C` record/config OBUs and track dimensions into `VtDecoderOptions`, creating a `CMVideoFormatDescription` with `SampleDescriptionExtensionAtoms.av1C`, and passing AV1 sample OBUs without AVCC/HVCC packing. The VT precise benchmark now has a decode-only AV1 mode, suite opt-in, and PSNR-Y verification against FFmpeg software NV12 reference. Actual macOS hardware decode, FFmpeg parity, and PSNR are not executed on this Windows host; AV1 encode is still explicitly unsupported. | Decode scaffolded; encode not implemented |
