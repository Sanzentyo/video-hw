...
- Benchmark input correction:
  - The integrated Vulkan runner now generates a codec-matched Annex-B decode input for the requested `--width`, `--height`, and `--frame-count` using FFmpeg software encoders (`libx264` / `libx265`) and passes that same input to both `video-hw` and FFmpeg Vulkan decode. This fixes the older Vulkan decode reports that used the fixed `sample-videos/sample-10s.*` input while computing throughput with the requested frame count.
  - Updated H.264 Vulkan report: `output/benchmark-backends-h264-1777968804.md`; generated decode input `output\benchmark-vulkan-decode-input-h264-320x180-30f.h264`. NVIDIA Vulkan remains PASS: decode 117.981 fps vs FFmpeg Vulkan 71.134 fps; encode 116.803 fps vs FFmpeg Vulkan 76.247 fps. Intel Vulkan H.264 remains unusable here: `video-hw` adapter unavailable, FFmpeg Vulkan decode exits `0xc0000005`, encode fails with `Function not implemented`.
  - Updated HEVC Vulkan report: `output/benchmark-backends-hevc-1777968807.md`; generated decode input `output\benchmark-vulkan-decode-input-hevc-320x180-30f.h265`. NVIDIA Vulkan remains PASS: decode 187.153 fps vs FFmpeg Vulkan 75.620 fps; encode 126.877 fps vs FFmpeg Vulkan 87.837 fps. Intel Vulkan HEVC direct ash now parses the generated `coded=320x180` stream but still fails at `vkGetVideoSessionMemoryRequirementsKHR returned no memory requirements`; FFmpeg Vulkan HEVC decode passes at 100.244 fps, while encode still fails with `Function not implemented`.
- DPB session sizing probe:
  - Added opt-in diagnostics `VIDEO_HW_VULKAN_HEVC_DECODE_SESSION_DPB_SLOTS=<n>` and `VIDEO_HW_VULKAN_HEVC_DECODE_SESSION_ACTIVE_REFS=<n>` for the direct ash HEVC decode submit probe. Defaults remain capability-derived.
  - `output/benchmark-backends-hevc-1777968951.md`: forcing `DPB_SLOTS=1` and `ACTIVE_REFS=1` still failed at `vkGetVideoSessionMemoryRequirementsKHR returned no memory requirements`.
  - `output/benchmark-backends-hevc-1777969016.md`: forcing `DPB_SLOTS=1` and `ACTIVE_REFS=0` also failed at the same point.
  - Interpretation: Intel's zero session-memory-requirements behavior is not caused by `video-hw` choosing the advertised maximum `maxDpbSlots=15` / `maxActiveReferencePictures=15` for the generated 320x180 IDR probe.
