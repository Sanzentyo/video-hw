...
- fMP4 metadata/report を JSON 等へ保存する利用者は `video-hw-fmp4` の `serde` feature を有効化する。`SampleEntry` は外部 runtime 型なので serde 出力には含めず、deserialization 後は `sample_entry=None` として扱う。保存用の codec/layout/parameter sets は `Mp4IndexSnapshot::track_descriptions` を使う。decode や `to_annexb()` が必要な場合は元ファイルを reader で開き直して `read_sample()` する。`DecodeDiagnostics` の `resolved_backend` を deserialize する場合は、保存時に使った backend feature を読み込み側でも有効化する
- async fMP4 reader は worker 経由の owned API を使う。`samples(track)` は `Vec<SampleMeta>` を返し、sync reader の borrow slice とは所有形態が違う。seek/read では `sample_at_pts_with_delta`、`gop_for_sample`、`read_sample`、`read_gop`、`read_segment`、`index_snapshot` を使う
- Vulkan HEVC の PSNR 回帰確認は `cargo +nightly -Zscript scripts/check_vulkan_hevc_psnr.rs` を使う。raw NV12 同士の比較で frame 単位の `psnr_y` 最小値を確認する
