...
use constants::{ApiVersion, Codec, FourCC, IoPattern, MfxImpl, PicStruct};
use decode::Decoder;
use encode::Encoder;
...
struct LoadedVpl(ffi::vpl);

// The generated symbol table only stores function pointers. It is safe to share after load.
unsafe impl Send for LoadedVpl {}
unsafe impl Sync for LoadedVpl {}

static LIBRARY: OnceCell<LoadedVpl> = OnceCell::new();
...
impl Loader {
    ...
    pub fn implementations(&self) -> Result<Vec<ImplDescription<'_>>, MfxStatus> {
...
impl<'a> FrameSurface<'a> {
    fn ensure_buffer_size(&mut self) {
        if self.buffer.len() != self.frame_size {
            self.buffer.resize(self.frame_size, 0);
        }
    }
    ...
    async fn read_nv12_frame(&mut self) -> Result<(), MfxStatus> {
        self.ensure_buffer_size();
        let bounds = self.bounds();
        let crop_h = bounds.crop_height as usize;
        let crop_w = bounds.crop_width as usize;
        ...
        for i_h in 0..crop_h {
            let source_offset = i_h * crop_w;
            let target_offset = i_h * pitch;
            let source = &buffer[source_offset..source_offset + crop_w];
            let target = &mut y[target_offset..target_offset + crop_w];
            target.copy_from_slice(source);
        }

        let uv_input_offset = crop_h * crop_w;
        let uv_rows = crop_h / 2;
        let uv_width = crop_w / 2;
        let uv_pitch = pitch / 2;
        for i_h in 0..uv_rows {
            let source_row =
                &buffer[uv_input_offset + i_h * crop_w..uv_input_offset + (i_h + 1) * crop_w];
            let u_row = &mut u[i_h * uv_pitch..i_h * uv_pitch + uv_width];
            let v_row = &mut v[i_h * uv_pitch..i_h * uv_pitch + uv_width];
            for x in 0..uv_width {
                let base = x * 2;
                u_row[x] = source_row[base];
                v_row[x] = source_row[base + 1];
            }
        }
    ...
    pub async fn read_raw_frame<R: Read>(
        ...
        let read_func = async {
            match format {
                FourCC::NV12 => self.read_nv12_frame().await,
...
impl<'a> Session<'a> {
    ...
    pub fn decoder(&self, params: MfxVideoParams) -> Result<Decoder<'_, '_>, MfxStatus> {
        Decoder::new(self, params)
    }

    // Get a new instances of a encoder tied to this session
    pub fn encoder(&self, params: MfxVideoParams) -> Result<Encoder<'_, '_>, MfxStatus> {
    ...
    ) -> Result<VideoProcessor<'_, '_>, MfxStatus> {
    ...
    pub fn implementation(&self) -> Result<MfxImpl, MfxStatus> {
        ...
        let implementation = MfxImpl::try_from(implementation as ffi::mfxImplType).unwrap();
...
pub fn get_library() -> Result<&'static ffi::vpl, libloading::Error> {
    ...
    if LIBRARY.set(LoadedVpl(lib)).is_err() {
