...
impl<'a: 'b, 'b> Decoder<'a, 'b> {
    ...
    fn queue_decode(
    ...
    ) -> Result<FrameSurface<'_>, MfxStatus> {
        let lib = get_library().unwrap();
        ...
        let mut sync_point: ffi::mfxSyncPoint = std::ptr::null_mut();
        let surface_work = work_surface
            .map(|s| s.inner as *mut _)
    ...
    /// Queues a decode operation and returns the output surface without waiting
    /// for synchronization. Call [`FrameSurface::synchronize`] before reading
    /// frame data or metadata.
    pub fn decode_async(
        &self,
        bitstream: Option<&mut Bitstream<'_>>,
        work_surface: Option<&mut FrameSurface<'_>>,
    ) -> Result<FrameSurface<'_>, MfxStatus> {
        self.queue_decode(bitstream, work_surface)
    }
    ...
    pub async fn decode(
        ...
        let mut output_surface = self.queue_decode(bitstream, work_surface)?;
    ...
    pub fn surface(&self) -> Result<FrameSurface<'_>, MfxStatus> {
        let lib = get_library().unwrap();
        let session = self.session.inner.0;
    ...
    pub fn reset(&mut self, mut params: MfxVideoParams) -> Result<(), MfxStatus> {
        ...
        let status: MfxStatus = unsafe { lib.MFXVideoDECODE_Reset(session, &mut **params) }.into();
    ...
    pub fn params(&self) -> Result<MfxVideoParams, MfxStatus> {
        ...
        let status: MfxStatus =
            unsafe { lib.MFXVideoDECODE_GetVideoParam(session, &mut **params) }.into();
...
mod tests {
    ...
    use crate::{
        bitstream::Bitstream,
        constants::{ApiVersion, Codec, ImplementationType, IoPattern},
        Loader,
    };
