...
fn main() -> Result<()> {
    ...
    if summary.min_y < args.min_psnr_y {
        bail!(
            "Vulkan HEVC PSNR below threshold: min_y={:.4} dB at frame {}, threshold={:.4} dB",
            summary.min_y,
            summary.min_y_frame,
            args.min_psnr_y
        );
...
fn parse_args() -> Result<Args> {
    ...
    let mut output_dir = PathBuf::from("output/vulkan-hevc-psnr");
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--offset-mode" => offset_mode = Some(next_value(&mut iter, "--offset-mode")?),
            "--skip-build" => skip_build = true,
            "-h" | "--help" => {
                println!(
                    "usage: cargo +nightly -Zscript scripts/check_vulkan_hevc_psnr.rs [--input FILE] [--output-dir DIR] [--ffmpeg PATH] [--width N] [--height N] [--min-psnr-y DB] [--offset-mode MODE] [--skip-build]"
                );
                std::process::exit(0);
            }
...
fn decode_with_vulkan(args: &Args, output: &Path) -> Result<()> {
    ...
    if let Some(mode) = &args.offset_mode {
        command.env("VIDEO_HW_VULKAN_HEVC_SLICE_OFFSET_MODE", mode);
    }
    run(&mut command, "decode_to_yuv vulkan hevc")
}
