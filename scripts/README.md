...
- Vulkan AV1 decode で `--verify` を付けると、各 `video-hw` Vulkan adapter に対して同じ生成入力を `scripts/check_vulkan_av1_psnr.rs` に渡し、FFmpeg software decode reference との `psnr_y_min >= 60 dB` を統合レポートの `video-hw PSNR verify` 行として記録する。
