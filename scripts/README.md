...
- 2026-05-06 の Windows/Intel-visible 環境では `--no-record-command-buffer` / `--record-mode barrier_only` / `begin_end` / `reset_end` は PASS、`first_decode` 以上は isolated test process が `STATUS_ACCESS_VIOLATION` で終了する状態。実submit/readbackへ進む前のdriver command-chain切り分け用として扱う。
