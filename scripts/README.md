...
- Vulkan HEVC encode は NVIDIA adapter 上で実験的な IDR-only production path を測定できる。runner は FFmpeg `hevc_vulkan` で同サイズの parameter/header sample を生成し、名前/vendor/device idで対応したFFmpeg Vulkan adapter番号を使って `VIDEO_HW_VULKAN_HEVC_ENCODE_PARAMETER_SAMPLE_PATH` を自動設定する。現状は1回の `flush` 内で Vulkan video session / session parameters を再利用するが、長寿命encoder sessionと参照フレーム/GOP encodeは未実装なので性能 parity は未達。失敗adapterや非公開adapterも report に残す。診断用には `cargo +nightly -Zscript scripts/check_vulkan_hevc_encode_probe.rs --adapter-index <n>`、または `cargo test -p video-hw-backend-vulkan --features backend-vulkan live_hevc_encode_session_bootstrap_reports_submit_feedback -- --ignored --nocapture` を明示実行する。
