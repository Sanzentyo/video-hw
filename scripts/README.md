...
- `--vulkan-av1-lag-in-frames 25` のような alt-ref/show-existing を含む入力は、現在の Vulkan AV1 実装の未対応範囲を明示的に測る負荷として使う。2026-05-06 の NVIDIA 実測では DPB slot replay は bounded slot に収まり、metadata decode は readback なし command submit として PASS する。一方、PSNR/NV12 readback は single command scope の最後に DPB image をまとめて読む設計のため、16 表示フレーム / 17 decode command の入力で `readback frame count is too small` として FAIL する。
