...
- AV1 入力生成は `libaom-av1` が無い環境では `libsvtav1` / `librav1e` にフォールバックする。レポートには `preflight_decode` による `decode_hardware_acceleration` / `decode_usable_in_current_runtime` も記録する。AV1 decode 比較では `video-hw` と FFmpeg の両方が NV12 raw frames を書き出す。
