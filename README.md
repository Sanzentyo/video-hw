...
- `VIDEO_HW_VULKAN_HEVC_ENCODE_DST_PREFIX_MODE=none|zero|parameter-sample`（`ffmpeg` alias 可）で `dst_prefix` 領域を実際に初期化できます。FFmpeg 生成 320x180 stream の先頭 256 bytes を prefix へ書いても `vkEndCommandBuffer failed` は不変でした。
