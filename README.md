...
- FFmpeg と同じく encode image view には identity `VkSamplerYcbcrConversion` を接続します。また `VIDEO_HW_VULKAN_HEVC_ENCODE_DPB_BARRIER_MODE=with|none` で DPB image の明示 barrier を切替可能です。FFmpeg 風 `none` と identity YCbCr view を使っても、現環境の NVIDIA Vulkan HEVC encode submit はまだ `vkEndCommandBuffer failed` です。
