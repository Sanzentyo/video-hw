...
- `VIDEO_HW_VULKAN_HEVC_ENCODE_DST_PREFIX_MODE=none|zero|parameter-sample`（`ffmpeg` alias 可）で `dst_prefix` 領域を実際に初期化できます。FFmpeg 生成 320x180 stream の先頭 256 bytes を prefix へ書き、`parameter_vui_safety=preserve` を併用した FFmpeg 風条件では NVIDIA Vulkan HEVC encode submit が `Ready(bytes_written=47)` まで進みます。ただし production encoder としてはまだ有効化しておらず、出力bitstreamのFFmpeg decode/PSNR検証も未完了です。
