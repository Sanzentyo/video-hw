...
- HEVC decode は `VK_KHR_video_decode_h265` を直接使う ash-level path です。HEVC encode は `VK_KHR_video_encode_h265` を直接使う実験的 IDR-only path です。現時点では参照フレーム/GOP encodeや persistent session reuse は未実装で、性能比較では FFmpeg `hevc_vulkan` より大きく遅れます。
