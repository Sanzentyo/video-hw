...
- HEVC decode は `VK_KHR_video_decode_h265` を直接使う ash-level path です。HEVC encode は `VK_KHR_video_encode_h265` submit path の調査中で、現時点では未実装です。
...
- 追加で `VIDEO_HW_VULKAN_HEVC_ENCODE_BEGIN_REFERENCE_SLOT_MODE=slot-minus-one|none` と `VIDEO_HW_VULKAN_HEVC_ENCODE_SETUP_REFERENCE_SLOT_MODE=slot-zero|none` により begin/setup reference slot を個別に無効化できます。`sample` / `empty-template` の8ケースでも `vkEndCommandBuffer failed` は不変でした。
...
- FFmpeg parity 追従として SPS/VPS のゼロHRD payload pointerを保持し、`rate_control_mode=cbr|vbr` では slice `constant_qp=0` / `slice_qp_delta=-26`（PPS init QP 0時）へ切り替えます。ただし現環境の `rate_control_mode=cbr + control_mode=ffmpeg` 追試でも `vkEndCommandBuffer failed` は不変で、`empty-template` の1920x1088整列ケースも失敗します。
