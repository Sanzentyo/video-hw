...
use crate::{
    BackendError, CapabilityReport, Codec, DecodeSummary, EncodedPacket, Frame, VideoDecoder,
    VideoEncoder,
};
...
pub struct NvidiaDecoderAdapter {
    codec: Codec,
...
impl NvidiaDecoderAdapter {
    pub fn new(codec: Codec) -> Self {
        Self {
            codec,
            assembler: StatefulBitstreamAssembler::with_codec(codec),
            last_summary: DecodeSummary {
                decoded_frames: 0,
                width: None,
                height: None,
...
impl VideoDecoder for NvidiaDecoderAdapter {
    ...
    fn push_bitstream_chunk(
        ...
        let _ = self.assembler.push_chunk(chunk, self.codec, pts_90k)?;
        Err(BackendError::UnsupportedConfig(
            "nvidia-sdk bridge is not wired yet".to_string(),
        ))
    }
